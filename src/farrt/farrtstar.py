...
from farrt.plot import plot_potential_field
...
class FARRTStar(RRTStar):
  ...
  def do_tree_severing(self, previous_plan: list[Node]) -> tuple[MultiPoint, MultiPoint, MultiPoint]:
    ...
    closest_parents: MultiPoint = MultiPoint()
    ...
    if self.gui:
      print('Render tree after severing - closest parents in white')
      self.render(visualize=True, save_frame=True, extra_points=closest_parents)
  ...
  def apply_potential_field(self, freed_pts: MultiPoint, /,*,field_force:float=None) -> MultiPoint:
    ...
    def get_movement_from_potential_field(pt: Point) -> tuple[float,float]:
      dx,dy = self.potential_field[math.floor(pt.y), math.floor(pt.x)] * field_force
      return dx,dy
  ...
  def do_farrt_rewiring(self, closest_parents: MultiPoint):
    """
    Rewire the free points into the tree

...
class FARRTStar(PartiallyObservablePlanner):
  ...
  def __init__(self, *args, **kwargs) -> None:
    ...
    self.farrt_tree: MultiPoint = MultiPoint()
  ...
  def do_first_plan(self) -> None:
    ...
    self.planned_path = []
    print(f'Do first farrt plan! {pt2tuple(self.x_start_pt)} -> {pt2tuple(self.x_goal_pt)}')
  ...
  def extract_path(self, *, endpoint: Point, root: Point, reverse:bool = True) -> list[Node]:
    """
    Extracts the path from the root of farrt tree to the endpoint
